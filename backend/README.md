...
| 기능           | 익명 사용자         | 인증 사용자 |
| -------------- | ------------------- | ----------- |
| 백테스트 실행  | O (Rate Limit 적용) | O           |
| 결과 DB 저장   | X                   | O           |
| 결과 목록 조회 | X                   | O           |
| 결과 삭제      | X                   | O           |

익명 사용자는 백테스트 실행 후 결과를 즉시 확인할 수 있지만,
결과는 저장되지 않습니다. 로그인하면 결과 저장 및 목록 조회가 가능합니다.

**익명 사용자 정책의 효과:**

- **서버 효율**: 익명 사용자 결과를 저장하지 않아 불필요한 DB 부하 방지
- **로그인 유도**: 결과 저장/관리 기능을 통해 자연스러운 회원가입 유도
