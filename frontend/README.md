...
MarketPage 사이드바에 기술적 지표와 매매 신호를 표시합니다.

### 지표 카드 (TechnicalIndicatorsCard)

| 지표            | 표시 내용                      |
| --------------- | ------------------------------ |
| RSI             | 값 + 상태 (과매수/과매도/중립) |
| MACD            | MACD, Signal, Histogram + 상태 |
| MA              | MA(20), MA(50) 값              |
| Bollinger Bands | Upper, Middle, Lower 밴드 값   |

- 로딩/에러 시 스켈레톤 UI 표시
- API 실패 시에도 차트는 정상 동작

### 신호 요약 카드 (SignalSummaryCard)

- 종합 매매 신호: Buy / Sell / Neutral
- 신호 강도: Strong / Moderate / Weak
- 신호 근거 목록 (RSI, MACD, MA 크로스, BB 돌파 등)

### Rate Limit 처리

- API 요청 한도 초과(429) 시 서버의 `Retry-After` 헤더 값으로 카운트다운 표시
- 헤더가 없을 경우 기본값 60초 적용
- 카운트다운 완료 후 자동 재시도
